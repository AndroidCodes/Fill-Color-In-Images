...
public class ColorGFX extends SurfaceView implements Runnable {
     ...
     * Constructor.
     */
    public ColorGFX(Context context, int width, int height, ProgressBar pbFloodFill) {
        super(context);
    ...
    public ColorGFX(Context context, int width, int height, boolean isSavedData,
                    Bitmap savedPathBitmap, ProgressBar pbFloodFill) {
        super(context);
    ...
    public void run() {
        ...
        while ( !mOurThread.isInterrupted() ) {
            ...
            // If a new image3 hasn't been requested via the previous/next
            // buttons...
...
//
//                    // If we need to resize the image3 because the width or
//                    // height is too
...
//                // happen. We need to create a scaled copy of the image3 by allocating
//                // more memory.
//                if (scaleFailed) {
//                    int newWidth = (int) (picture.getWidth() / resizeRatioHeight);
//                    int newHeight = (int) (picture.getHeight() / resizeRatioHeight);
//
//                Bitmap paintMap = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(getResources(), resPaintId, null), imageWidth, imageHeight, true);
//
                Bitmap paintMap =  svgToBitmap(getResources(),resPaintId, imageHeight, imageWidth);
...
//                }
                ...
                if (targetColor == Color.TRANSPARENT) {
                    ...
                    pbFloodFill.setVisibility(View.VISIBLE);
...
//          svg.renderToCanvas(canvas);
    ...
    private class LoadViewTask implements Runnable {
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!isThreadBroken) {
                        ...
                        // Save the flood filled image3 so it can persist in
                        // future flood fills and path draws.
                        // mPaintBitmap = picture;

                        // Draw the newly filled image3 onto the path canvas.
        ...
        public void floodFill(Point node, int targetColor,
            ...
            // Initialize the arrays according to the image3 metrics.
            ...
            if (target != replacement) {
                ...
                do {
                    ...
                    // bitmap (AKA hasn't hit the end of the image3 and hasn't
